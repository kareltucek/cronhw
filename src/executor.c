 ...
 * Functions in this file are triggered by the daemon. For more information on
 * the data flow see the main/daemon documentation.
 */
 ...
 */
bool
contains(intlist_t *intlist, int a)
{
...
/*
 * Returns the field of supplemented time which corresponds to the ith field
 * of crontab format
 */
int
get_current_time(time_t t, int i)
{
	struct tm *info;
	info = localtime(&t);
	switch (i)
...
/*
 * This executes the supplemented task in a newly spawned shell. The output is
 * redirected to a temporary file, which is later read from the main thread.
 */
void
 ...
 */
void
handle_task(task_t task, time_t t)
{
	for (int i = 0; i < 5; i++)
	{
 ...
 */
void
execute_tasks(tasklist_t tasklist, time_t t)
{
