 ...
 * */
bool rb_str_cmp(RB_NODE_T(string_tree_t)* n1, RB_NODE_T(string_tree_t)* n2)
{
...
SLIST_LIST_T_GEN(intlist_t, int)
   ...
   * */
bool getline(int fd, char** lineptr)
{
 ...
 * */
void parse_ctx_set(parse_ctx_t* pctx, char* key, char* value)
{
 ...
 * */
void init_parse_ctx(parse_ctx_t* pctx)
{
//TODO: add default environment vars
  RB_INIT(&pctx->dict);
...
void clear_parse_ctx(parse_ctx_t* pctx)
{
 ...
 * */
char* eval(parse_ctx_t* pctx, char* varname)
{
  RB_NODE_T(string_tree_t) find, *res; 
  find.key = varname; 
  res = RB_FIND(string_tree_t, &pctx->dict, &find);
 ...
 * */
int preprocess_line_length(parse_ctx_t* pctx, char* line)
{
 ...
 * */
char* preprocess_line(parse_ctx_t* pctx, char* line)
{
 ...
 * */
void parse_range(char** line, intlist_t* list)
{
  char* ptr = *line;
  while(*ptr == ' ')
    ptr++;
 ...
 * supplemented list.
 * */
void parse_command(char* line, tasklist_t* list)
{
  task_t task;
  task_init(&task);
  task.cmdline = line;
 ...
 * */
void parse_line(parse_ctx_t* pctx, char* line, tasklist_t* list)
{
  char* ptr = line;
  while(*ptr != '\0')
  {
 ...
 * */
tasklist_t loadFromFile(char* filename)
{
